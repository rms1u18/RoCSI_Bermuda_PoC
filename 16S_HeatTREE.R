### 16S Bermuda Open ocean and coastal heat tree comparisons
...
setwd('OneDrive - University of Southampton/PhD/Bermuda/Coding_folders/Bermuda_16S_QIIME/exported')
...
otu_data <- left_join(otu_data, taxa_data,
                      by = c("#OTU ID" = "Feature ID")) # identifies cols with shared IDs
print(otu_data)

head(otu_data$Taxon)
tail(colnames(otu_data), n = 10)

# Lets remove any unassigned reads
otu_data <- otu_data[otu_data$Taxon != "Unassigned", ]
...
sample_data <- sample_data %>% filter(SampleID != "2677a-16S")
sample_data <- sample_data %>% filter(SampleID != "2945b-16S")
sample_data <- sample_data %>% filter(SampleID != "2626b-16S")
sample_data <- sample_data %>% filter(SampleID != "2888b-16S")
...
# obj <- filter_taxa(obj, taxon_names == "Bacteria", taxon_names == "Archaea"), subtaxa = TRUE)
print(obj)
...
obj$data$tax_abund <- calc_taxon_abund(obj, 
                                      "otu_props",
                                      cols = sample_data$SampleID) #all samples
...
# find the log ratio of mean abundances in the two groups - we'll set any non significant differences to 0 and re plot
...
obj %>%
  ...
  heat_tree(node_label = cleaned_names,
            ...
            node_color = log2_median_ratio, # difference between groups
            node_color_interval = c(-10, 10), # symmetric interval
            node_color_range = c("orange", "gray", "lightseagreen"), # diverging colors
            node_size_axis_label = "OTU count",
            node_color_axis_label = "Log 2 ratio of median counts",
            layout = "da", initial_layout = "re", # good layout for large trees
            title = "16S",
            output_file = "16S_OvC_HeatTREEsig.pdf")

# Playing with node labels for publication
set.seed(4)
obj %>%
  mutate_obs("cleaned_names", gsub(taxon_names, pattern = "\\[|\\]", replacement = "")) %>%
  taxa::filter_taxa(grepl(cleaned_names, pattern = "^[a-zA-Z]+$")) %>%
  heat_tree(node_label = cleaned_names,
            ...
            node_color_axis_label = "Log 2 ratio of median counts",
            layout = "da", initial_layout = "re", # good layout for large trees
            output_file = "16S_OvC_HeatTREEsig_pub.pdf")
